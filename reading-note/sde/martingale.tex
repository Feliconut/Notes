...
\begin{lemma}
	...
	\begin{proof}
		...
		\[
			E(X_m ; A) = E(X_n ; A)
		.\] 
		Let $ m\to \infty $. Then $E(X_m ; A) \to E(X;A)$ in $L^1$. Hence $X_n = E(X | \mathcal{F}_n)$.
\end{proof}
\begin{theorem}
...
\subsubsection{L1, L2, Lp}

Write $\overline{X} _n = \max_{m \le n} X_m^+$.
\begin{theorem}[Doob's L1 Inequality]
	\label{thm:DooblInequality}
	Let $X_n$ be a submartingale. Let $A = \{\overline{X} _n > \lambda\} $. Then
	\[
		\lambda P(A) \le E\left( X_n 1_A \right) \le E(X_n^+)
	.\] 
\end{theorem}
\begin{proof}
	Let $N = \inf \{m: X_m > \lambda\} \wedge n$, we have
	\[
		\lambda P(A) \le E(X_N 1_A) \le E(X_n 1_A) \le E(X_n^+)
	.\] 
	The first inequality is because $X_N > \lambda$ on $A$. The second inequality is due to Optional Stopping Theorem (Theorem~\ref{thm:OptionalStoppingThm}) and that $X_n = X_N$ on $A^c$. The last inequality is trivial.
\end{proof}

\begin{theorem}[Doob's Lp Maximal Inequality]
	\label{thm:DooblpMaximalInequality}
	Let $X_n$ be a submartingale. Then for all $1 < p \le \infty $,
	\[
		E\left( |\overline{X} _n|^p \right) 
		\le 
    ...
    That is,
    \[
\left\|\sup _{0<t<T}\left|X_t^{+}\right|\right\|_p \leq \frac{p}{p-1}\left\|X_T^{+}\right\|_p .
\]
\end{theorem}
\begin{proof}
	We work with $\overline{X} _n \wedge M$. Use layer-cake representation, then apply L1 inequality, and finally apply H\"older inequality:
	\begin{align*}
		E\left( |\overline{X} _n \wedge M|^p \right) 
		&= \int _0^\infty p \lambda^{p - 1} P\left(  \overline{X} _n \wedge M > \lambda \right) d \lambda  \\
		&\le  \int _0^\infty p \lambda^{p - 1} \lambda^{-1} \int _\Omega X_n^+ 1_{\overline{X} _n \wedge M > \lambda} dP d\lambda  \\
		&= \int _\Omega X_n^+ \int _0^{\overline{X} _n \wedge M}  p \lambda^{p - 2} d\lambda dP \\
		&= \frac{p}{p - 1} \int _\Omega X_n^+ |\overline{X} _n \wedge M|^{p - 1} dP \\
		&\leq \frac{p}{p - 1} (E (X_n^+)^p)^{\frac{1}{p}} (E|\overline{X} _n \wedge M|^{p})^{\frac{p - 1}{p}} 
	.\end{align*}
	Divide both sides by $(E|\overline{X} _n \wedge M|^{p})^{\frac{p - 1}{p}}$ and raise to $p$-th power, we obtain the desired inequality.
	The application of L1 inequality is justified because $P(\overline{X} _n \wedge M > \lambda) \le P (\overline{X} _n > \lambda)$.
\end{proof}

\begin{remark}
    This can be used to prove the Maximal Ergodic Theorem. See talk.
\end{remark}

\textbf{Counterexample.} In Doob's Lp maximal inequality one must have $ p > 1$. To see why this does not hold for $p = 1$, consider a simple symmetric random walk with $S_0 = 1$, $N = \inf \{n: S_n = 0\}$, $X_n = S_{N \wedge n} $. We have $E X_n = E S_{N \wedge n} = E S_0 = 1$, by Theorem~\ref{thm:EXTn}. Using hitting probabilities (optional stopping theorem), we have $P(\max_m X_m \geq M)= P(T_M < T_0) = 1 / M$. Thus, $E(\max_m X_m) = \sum_M P(\max_m X_m \geq M) = \infty$. The monotone convergence theorem implies that $E \bar X_n = E (\max_{m \leq n} X_m) \to E (\max _n X_m) = \infty$ as $n \to \infty$.

\begin{theorem}[$L^p$ convergence theorem]
    If $X_n$ is a martingale with \\$\sup E |X_n|^p < \infty$, where $p > 1$, then $X_n \to X$ a.s. and in $L^p$.
\end{theorem}
\begin{proof}
    (a.s.) $(EX_n^+)^p \leq (E|X_n|)^p \leq E |X_n|^p$. By Doob's martingale convergence theorem (Theorem~\ref{thm:DoobMartingaleConvergence}), $X_n$ converges a.s. and $E|X| < \infty$.

    ($L^p$) By taking $n \to \infty$ on both sides of the $L^p$ maximal inequality, we see the right hand side is uniformly bounded, so that $\max_m |X_m| \in L^p$. Sicne $|X_n - X| \leq 2 \max_m |X_m|$, by dominated convergence theorem, we have $X_n \to X$ in $L^p$.
\end{proof}

\subsubsection{Buckholder-Davis-Gundy Inequalities}

\subsubsection{Concentration Inequalities}


\subsection{Tail sigma fields and 0-1 laws}
...
\begin{remark}
    This can be generalized to $L^p$ convergence.
\end{remark}
...
This gives a very quick proof of
\begin{corollary}[Kolmogorov's 0-1 Law]
    Let $X_1, X_2, \dots$ be independent and suppose $A \in \mathcal{T}$, the tail $\sigma$-field. Then $P(A) = 0$ or $1$.
\end{corollary}
\begin{proof}
    Since $A \in \mathcal{T}$, $A$ is independent of $\mathcal{F}_n$ for each $n$. Thus $E(1_A | \mathcal{F}_n) = E(1_A) = P(A)$. Levy's 0-1 law tells us that $P(A) = 1_A$ which is in $\{0, 1\}$.
\end{proof}

Next: Lp convergence, (concentration) inequalities, upcrossing inequality, square integrability, continuous martingale