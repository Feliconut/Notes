...
\begin{theorem}
	...
	\[
		\left\langle \Psi \right| \phi(x_1) \ldots \phi(x_n) \left| \Omega \right\rangle \neq 0
	,\] 
...
\begin{itemize}
	...
	\item The nonzero matrix elements corresponding to the possible on-shell intermediate mass-$m$ states that can be formed. For each such term one can demonstrate a pole and a factorization.
\end{itemize}
...
Here we follow the demonstration in \autocite[Chapter~27.6]{schwartzQFT2014}, for gluon scattering amplitudes (massless spin-1).
...
This case is also special in that the matrix elements can only depend on the inner products of the respective spinors, and combining with a dimensional analysis argument, one can determine the expression of all 4-point amplitudes up to a coefficient.
...
\textbf{On-shell intermediate states.} For interval $a \ldots b$ that includes $j$ but not $i$, consider its intermediate momenta $\hat{P} (z) = - z i \rangle [j + \sum_{k = a}^b k \rangle [k$. Remarkably, for unique choice of $z$:
\[
z_{a, b}^{\star}=\frac{\left(p_a+\cdots+p_b\right)^2}{\langle i a\rangle[a j]+\cdots+\langle i b\rangle[b j]}
,\] 
we get $\hat{P} ^2(z^*) = 0$. Now, this state is an on-shell state and has a nonzero matrix element, given by inner product.

\textbf{Applying Polology.} Our proof for polology is complex-analytic, so it also works for complex in and out momenta, except that our complete set of intermediate states is still real on-shell. Yet $\hat{P} (z^*)$ is real and on-shell. Hence polology can be applied, and we get the following local factorization:
\begin{align*}
...
\begin{itemize}
    \item In the proof of polology it is also guaranteed that the poles can be only simple poles, and we combine this with the further assumption that analyticity holds everywhere other than those poles.

    \item We also rely on the polology fact that each pole must correspond to an on-shell intermediate state. Then this procedure will exhaust all possible poles, and in particular we have finitely many poles.
