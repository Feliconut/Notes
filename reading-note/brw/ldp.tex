...
\begin{enumerate}
    \item $E X_1 = 0$. This can be easily removed by centering the random variable.
    \item $X_1$ has both positive and negative tails. The one-sided case can be handled by a very simple proof which we ignore for now.
\end{enumerate}
...
\begin{enumerate}[resume]
    \item $x = 0$. The proof can be easily extended to the case $x \neq 0$.
\end{enumerate}
...
Next, we give the proof for $x \neq 0$. We can use the same proof as above, but we need to be careful about the change of measure. We have
