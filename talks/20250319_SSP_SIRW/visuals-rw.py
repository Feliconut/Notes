...
class sirwlocal:
    ...
    plt.text(3, -1, r'$L^X(y, 60)$', fontsize=12)
    plt.title('Local Time of SIRW')
    
    # plt.show()
    plt.savefig('figures/sirwlocal.png', transparent=True)
...
class sirwplotrescaled:
    ...
    plt.savefig('figures/sirwrescaled.png',transparent=True)
...
class sirwlocalrescaled:
    ...
    plt.savefig('figures/sirwlocalrescaled.png', transparent=True)
# %%
class goodevent:
    local_times = SIRWlarge.local_times
    # Fill in missing values
    ...
    # plt.ylim(-15, 5)
    # plt.text(10, -2, r'$L^W(y, 30)$', fontsize=12)
    # plt.title('The good event')
    # plt.savefig('figures/sirwlocalrescaled.png')
    # hide axises and all borders
    # draw a box from -7 to 7, 0 to 14
    plt.plot([-7, 7], [0, 0], color='green', linestyle='dashed')
    plt.plot([-7, -7], [0, 14], color='green', linestyle='dashed')
    plt.plot([7, 7], [0, 14], color='green', linestyle='dashed')
    plt.plot([-7, 7], [14, 14], color='green', linestyle='dashed')
    plt.ylim(-1, 15)
    # print text on top and left
    plt.text(0, 13, r'$2K \sqrt{n}$', fontsize=12)
    plt.text(-7, 7, r'$K \sqrt{n}$', fontsize=12)
    # fill light green inside box
    ...
    # in center of box: L(*, \floor{nt})
    plt.text(3, 7, r'$L^W(\cdot, \lfloor nt \rfloor)$', fontsize=12)
    plt.savefig('figures/goodevent.png', dpi=300, transparent=True)
