...
\begin{proof}
	...
	Because $D_\alpha$ has content zero, there exists finitely many cells $\left\{ E_j \right\}_{j = 1}^{r} $ such that $D_\alpha$ is contained in the union of $E_j$ and $ \sum_{j=1}^{m} c(E_j) \le \alpha $. 
	Without loss of generality we may assume that $E_j$ are open cells. Since we are working in $\mathbb{R}^1$, we may assume the $E_j$ are mutually disjoint, since the union of any two non-disjoint open cells in $\mathbb{R}^1$ is yet another open cell.
	...
	Now $I \setminus \bigcup_{1\le j \le r} E_j$ is also a union of finitely many disjoint cells. Combining these cells with $\{E_j\} $ we obtain a partition $P = \left\{ I_j \right\}_{j = 1}^N $ of $I$ where  $I_j = E_j$ for  $0\le j\le r$, so $D_\alpha \subset \bigcup_{j = 1}^r I_j$, $\sum_{j=1}^{r} c(I_j) < \alpha $, $r \le N$.

	Now the union $\bigcup_{j = 1} ^r I_j$ is open, so its complement $\bigcup_{j = r+1} ^N I_j$ is closed. Since $I$ is bounded, by Heine-Borel,  $\bigcup_{j = r+1} ^N I_j$ is compact.
	...
	.\] 
	For each $x \in \bigcup_{j = r+1} ^N I_j$, we have $\overline{\delta} (x) > 0$, because the limit $\omega_f(x) < \alpha$. Since we are working on $\mathbb{R}^1$, $B_{\overline{\delta}(x)} (x) =: B(x)$ is an open box. By compactness, there exists finitely many points $x_1, \ldots, x_n \in \bigcup_{j = r+1} ^N I_j$ where $\left\{B(x_i)\right\}_{i = 1}^n $ is an open cover of $\bigcup_{j = r+1} ^N I_j$.
...
\begin{proof}
	...
	%.\] 
	Fix $\varepsilon>0$ and let $\alpha = \varepsilon \left(c(I)+2\|f\|_I\right)^{-1}$. Hence for some partition $P$ of $I$ and any refinement $P'$ of $P$,
	\[
...
\begin{proof}
	...
	Now by definition of $D_\alpha$, whenever $J \in P'$, $m(J) > 0$ and $J \cap D_\alpha \neq  \emptyset $, $J$ must contain some neighborhood of $x$. Therefore  $\sup_J f - \inf_J f \ge \omega_f(x) \ge \alpha$. Applying this to the Riemann sum, we have
	\[
