...
\begin{enumerate}
	\item for all $n$ and all distinct  $k, l$, 
		\[
...
\item Also, for any $n, k$, $A_{n+1, l} \subset A_{nk}$ if and only if \[
2k \le l \le 2k+1
.\] 
\end{enumerate}

Now define the sequence $(x_n)_{n=0}^{\infty }$ as
\begin{align*}
	x_0 &= \begin{cases}
		0 & x \in A_{00} \\
		1 & x \in A_{01}
	\end{cases}\\
	x_n &= \begin{cases}
		0 & x \in A_{n, 2k} \text{ where } x \in A_{n -1, k}\\
		1 & x \in A_{n, 2k+1} \text{ where } x \in A_{n -1, k}\\
	\end{cases}
\end{align*}

Now the function $F$ defined below is a bijection between all 0-1 sequences that are not fully 1 and  $[0,1)$:
 \begin{align*}
	F: \{0,1\}^\omega\setminus (1)_{n\in\N} &\longrightarrow [0,1) \\
	(x_n)_{n\ge 0} &\longmapsto F((x_n)_{n\ge 0}) = \sum_{n=0}^{\infty} \frac{x_n}{2^{n+1}}
.\end{align*}

Now we introduce a function $G $:
\begin{align*}
	G: \{0,1\}^\omega\setminus (1)_{n\in\N} &\longrightarrow [0,1) \\
	(x_n)_{n\ge 0}  &\longmapsto G((x_n)_{n\ge 0} ) = \sum_{n=0}^{\infty} \frac{2x_n}{3^{n+1}} 
.\end{align*}
We verify that $G$ is a bijection between all not-all-one 0-1 sequences and the cantor set. Then we have a bijection between $[0,1)$ and the cantor set.
\end{proof}
...
\begin{proof}
	...
	3^n = (1+2)^n \ge  1 + 2n
	\]
	and by Archimedean property we can choose $n$ such that  $n> \frac{\max(\frac{1}{x-a}, \frac{1}{b-x}) - 1}{2}$.
