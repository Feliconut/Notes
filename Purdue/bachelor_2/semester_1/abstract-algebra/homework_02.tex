...
\begin{proof}
	We use property 4 to pick a left inverse $i(x)$ for each element $x \in G$, where $i(x) * x = e$. We use a right-associative convention to avoid cumbersome notation, so $x*y*z$ always means $x*(y*z)$ rather than $(x*y)*z$.

First we show that $e$ is also a right identity.

	\begin{align*}
		x*e &= x*i(x)*x &\text{left inverse} \\
		    &= e*x*i(x)*x &\text{left identity} \\
		    &= \left( i(i(x))*i(x) \right) *x*i(x)*x &\text{left inverse} \\
		    &=  i(i(x))*\left( i(x)  *x \right) *i(x)*x &\text{associativity} \\
		    &=  i(i(x))*e  *i(x)*x &\text{left inverse} \\
		    &=  i(i(x))*\left( e  *i(x) \right) *x &\text{associativity} \\
		    &=  i(i(x))* i(x)  *x &\text{left identity} \\
		    &=  \left( i(i(x))* i(x) \right)   *x &\text{associativity} \\
	...
	\begin{align*}
		x * i(x) &=  e * \left( x * i(x) \right) & \text{left identity} \\
			 &= \left( i(i(x))*i(x) \right) * (x * i(x)) &\text{left inverse}\\
			 &= i(i(x))*((i(x)*x)*i(x)) &\text{associativity} \\
			 &= i(i(x))*(e*i(x)) &\text{left inverse} \\
			 &= i(i(x))*i(x) &\text{left identity} \\
			 &= e & \text{left inverse}
	.\end{align*}
...
Finally, subgroups of order 3. We write such a group as $\{e,a,b\} $ where $e$ is the identity. Note that $ab \neq a$, otherwise we have $ab = ae$ and $b = e$ by cancellation, which contradicts the assumption that  $a,b,e$ are distinct. By the same reasoning $ab \neq  b$, therefore $ab = e$. By the same reasoning we have $ba = e$. Therefore $b = a^{-1}$. Hence we are looking at two distinct elements from $S_3$ that are each other's inverse. There is only one such pairs in $S_3$:
\begin{enumerate}
	\item $\{(231),(312)\} $ 
\end{enumerate}
Therefore $\{e, (231),(312)\} $ is a subgroup of $S_3$ with order 3.

By Lagrange's Theorem, there's no other proper subgroups with higher order, since $4$ and $5$ do not divide $6$. The last subgroup would be order $6$, which is $S_3$ itself.
\end{proof}
...
\begin{proof}
We prove the contrapositive. Assume $G$ is not cyclic. Then there exists some $a \in G$ such that $\left\langle a \right\rangle $ is a proper subset of $G$. Because $\left\langle a \right\rangle $ is a cyclic group, it by definition is a proper subgroup of $G$.
...
\begin{align*}
	...
	&\in x^{-1} H x
.\end{align*}
Because $H$ is a subgroup of $G$, $h_n h_m \in H$. Because $x$ is arbitrary, $nm \in \bigcap_{x \in G} x^{-1} H x = N$. Hence $N$ is closed under multiplication.
...
\begin{proof}
	...
	Now for $y, z \in M$, whenever $f_x(y) = f_x(z)$, we have
	\begin{align*}
		f_x(y) &= f_x(z) \\
		x^{-1} yx &= x^{-1} zx\\
		x(x^{-1} yx)&= x(x^{-1} zx) \\
		...
		y &= z
	.\end{align*}
We have prove that $f_x$ is injective. Now we prove that $f_x$ is surjective. Fix any $y \in M$. Using the inverse axiom, we have $x^{-1}  \in M$. Using the assumption, we have $\left( x^{-1}  \right) ^{-1} M x^{-1} \subset M$, hence $\left( x^{-1}  \right) ^{-1} y x^{-1} \in  M$. Now
\begin{align*}
