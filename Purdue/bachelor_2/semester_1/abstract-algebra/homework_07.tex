...
\begin{proof}
	By Cauchy Theorem, there is an element $a \in G$ such that $o(a) = 11$. We claim that $\left\langle a \right\rangle \triangleleft G$.
	...
	\[
		...
		= \frac{\left| g^{-1} \left\langle a \right\rangle g \right| \left| \left\langle a \right\rangle  \right| }{\left| (g^{-1} \left\langle a \right\rangle g) \cap \left\langle a \right\rangle  \right| }
		= 11^2 > 99
	.\] 
...
\end{proof}
\begin{correction}
	We explicitly construct an isomorphism. Let 
\[
		G_2 = \{a_2 ^m b_2^n: 0\le m\le 2, 0\le n\le 6\} 
	.\]
	\[
		G_4 = \{a_4 ^m b_4^n: 0\le m\le 2, 0\le n\le 6\} 
	.\] 

	We need $\varphi$ is a hoomorphism:
	\[
		\varphi(a_2^{m_1}b_2^{n_1} a_4^{m_2}b_4^{n_2}) = \varphi(a_2^{m_1}b_2^{n_1})\varphi( a_4^{m_2}b_4^{n_2})
	.\] 
	Compute the lhs: \[
		\varphi(a_2^{m_1}b_2^{n_1} a_4^{m_2}b_4^{n_2}) = \varphi(a_2^{m_1+m_2}b_2^{n_1 2^{m_2} + n_2})
	.\] 
	We define
	\begin{align*}
		\varphi: G_2 &\longrightarrow G_4 \\
		a_2^m b_2^n &\longmapsto \varphi(a_2^m b_2^n) = a_4^{2m}b_4^{4n}
	.\end{align*}
\end{correction}
...
\begin{proof}
	...
	Rearrange:
\[
		n_i= \left( n_{i-1}^{-1}n_{i-2}^{-1}\ldots n_1^{-1} \right)  n_1'n_2'\ldots n_i' \ldots n_k' \left( n_{k}^{-1} n_{k-1}^{-1} \ldots n_{i+1}^{-1} \right) 
	.\]
	We can group $n_1n_1'$ and $n_k'n_k$ to obtain
\[
		n_i\in  \left( n_{i-1}^{-1}n_{i-2}^{-1}\ldots n_2^{-1} \right)  N_1 (n_2'\ldots n_i' \ldots n_{k-1}')N_k \left( n_{k-1}^{-1} \ldots n_{i+1}^{-1} \right) 
	.\]
	Since $N_1$ and $N_k$ are normal subgroups of $G$, they commute with group items. Hence
\[
		n_i\in  N_1\left( n_{i-1}^{-1}n_{i-2}^{-1}\ldots n_2^{-1} \right)  (n_2'\ldots n_i' \ldots n_{k-1}')\left( n_{k-1}^{-1} \ldots n_{i+1}^{-1} \right) N_k
	.\]
...
\[
	...
	.\]
	But we also have $n_i (n_{i}')^{-1} \in N_i$. Therefore by assumption 2, \[
		n_i = n_i'
	.\]
...
\begin{proof}
	...
	There exists $a \in G$ with order $11$. So $\left\langle a \right\rangle \subset G$. Then $\left\langle a \right\rangle \triangleleft G$ by a counting argument.

	Alternative method: use Sylow Third Theorem: There is only one Sylow $11$-subgroup of $G$.

	$|G / \left\langle a \right\rangle | = 9$, Hence $G / \left\langle a \right\rangle $ which is abelian.

	Suppose $b \in G / \left\langle a \right\rangle $. Such an element exists of order $3$.

	Since $\left\langle a \right\rangle  \triangleleft G$ then $b^{-1} ab = a^i$ for some $1\le i\le 10$. Now using conjugacy argument, $i^3 \equiv 1 \pmod 11$. Thus $i = 1$. Hence $ab = ba$. Then $|Z(G)| \ge 33$.
\end{proof}
