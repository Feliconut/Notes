\lecture{8}{Mon 19 Sep 2022 10:46}{homomorphism theorems}
...
\begin{proof}
	...
	\begin{enumerate}
		...
		$ab^{-1} \in H$. So $H \le G$ by the subgroup criterion.
	\item Since  $K = \operatorname{ker} \varphi$, we have $a \in K \implies \varphi(a) = e' \in H'$, so $a \in H$, so $K \subset H$.
	\item Next, since $K \triangleleft G$, we have $K \triangleleft H \subset G$. We cinsider the restricted map
		\begin{align*}
			\varphi|_H: H &\longrightarrow H' \\
			h &\longmapsto \varphi|_H(h) = \varphi(h)
		.\end{align*}
	So this map is surjective from $H$ to $H'$, with $\operatorname{ker} \varphi_|H = K$. Then by First Homomorphism Theorem, we have \[
		H / K \cong H'
	.\] 
\item When $H'\triangleleft G'$, then whenever, $g \in G$, 
	\[
	\varphi(g^{-1} Hg) = \varphi(g)^{-1}\varphi(H)\varphi(g) = \varphi(g)^{-1}H'\varphi(g) \subset H'
.\] 
So $g^{-1} Hg \subset H$. Hence $H \triangleleft G$.
	\end{enumerate}
\end{proof}

When $H \le G$ and $N \triangleleft G$, consider \[
	H N = \{hn \in G: h \in H, n \in N\} 
.\] 
\begin{claim}
	$H N \le G $.
\end{claim}
\begin{proof}
	Observe that whenever $h_1, h_2 \in H$ and $n_1, n_2 \in N$, one has 
	\begin{align*}
		(h_1n_1)(h_2n_2)^{-1} 
