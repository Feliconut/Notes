...
\begin{proof}
	...
	Let $x < T_n $ and now we compute $A(x) = \prob{X_2 < X^*_{[x,n-1]}}\prob{x<X^*_{[\tilde\cdot, n-2]}} V(\tilde \cdot, n-2) = \prob{X_2 < X^*_{[\tilde\cdot, n-2]}} \prob{x < X^*_{[\tilde \cdot, n-2]}} V(\tilde\cdot, n-2) = A(0)\prob{x < X^*_{[\tilde \cdot, n-2]}} $, which monotonically decrease wrt $x$. $B(x) = B(0) \prob{x < X^*_{[\tilde \cdot, n-1]}}$ also monotonically decrease wrt $x$.  $A(x) < B(x)$ follows from the lemma:
	...
	Now let $x \ge T_n$. Then $A(x) = \prob{X_2 < X^*_{[x, n-1]}} x^{n-1} = x^n$, which monotonically increase up to $1$. $B(x)$ still monotonically decrease. We can also verify that $A(x)$ and $B(x)$ are continuous. Thus we can conclude that there exists a unique point $\alpha \in [T_n, 1]$ such that $A(x) > B(x)$ iff $x > \alpha$ for all $x \in [0,1]$.
\end{proof}
