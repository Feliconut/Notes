...
&\qquad + \prob{X_n \max} \int_0^1 \prob{X_1, \ldots, X_{n-1} < T | X_n = x \text{ is max}} f_{\max \text{Unif}(0,1)}(x) \,dx
\end{split}
\\
\begin{split}
&=\sum_{i=1}^{ n -1}\int_T^1 \prob{X_1,\dots,X_{i-1}<T \cap X_{i+1},\dots,X_n\leq x} f_{\text{Unif}(0,1)}(x) dx \\
&\qquad +\frac{1}{n} \int_0^1 \prob{X_i<T | X_i<x}^{n-1} f_{\max \text{Unif(0,1)}}(x) \,dx
\end{split}
\\
