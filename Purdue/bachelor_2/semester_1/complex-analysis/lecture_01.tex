\lecture{1}{Thu 25 Aug 2022 12:01}{lecture}
...
Raise to power.
\begin{itemize}
	\item $\left( \cos \theta + i \sin \theta \right) ^{n} = e^{i n \theta} = \cos n \theta + i \sin n \theta$, $n$ integer
	\item An application
		\begin{align*}
...
We may define complex exponentiation by extending $\theta$ from real number to complex number.
...
\begin{align*}
	...
	\intertext{$r = 1$ by comparing abs vlaues.}
	i n \theta &= 2 \pi i k\\
...
\begin{example}
	\begin{align*}
		&n=1 &z=1 \text{one sol.}\\
		&n=2 &z=1,-1 &z^{2}-1 = 0\\
		&n=3 &z^3-1 = 0 &z=1, z_2,z_3 \text{are solution of } z^2+z+1 = 0. $z_{2,3} = \frac{1}{2} \pm \frac{\sqrt{-3} }{2} = -\frac{1}{2} \pm i \frac{\sqrt{3} }{2}$.\\
		&n=4 &z^{4}-1 = 0 &z = 1,-1,i,-1
	.\end{align*}

\end{example}

\subsubsection{More general solution}

We solve the equation \[
z^n = w, w \text{given complex number}.
\] 
\begin{proof}[Solution]
\begin{align*}
