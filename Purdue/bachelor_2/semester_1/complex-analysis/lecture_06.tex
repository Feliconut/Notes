...
\begin{proof}
	\begin{enumerate}
		...
		.\] 
	\item Locally, $u = \operatorname{Re}  F$, then $u(f) = \operatorname{Re} F(f)$. $F(f)$ is analytic, since $F$, $f$ are.
	\end{enumerate}
\end{proof}
...
\begin{example}
	\begin{itemize}
		\item 
	$f(z) = z^2$ (nonexample) increases angle by factor of $2$ at $0$.
...
\begin{remark}
	to find out if $P$ and $Q$ have common roots is simple (GCD algorithm). TO factorize a polynomial is difficult. The same is for integers.
\end{remark}
