...
When running our algorithm, we fix some particular $\varepsilon$ and take infimum over $x$. We want to be assured that dropping the infimum over $\varepsilon$ when calculating $\alpha$ gives a good enough approximation. This motivates the next proposition:
\begin{proposition}
	We have the bound
	\[
		 \operatorname{inf}_x
...
\begin{theorem}[Berry-Esseen Inequality]
	3.4.9. Let $X_1, X_2, \ldots$ be i.i.d. with $E X_i=0, E X_i^2=\sigma^2$, and $E\left|X_i\right|^3=$ $\rho<\infty$. If $F_n(x)$ is the distribution of $\left(X_1+\cdots+X_n\right) / \sigma \sqrt{n}$ and $\mathcal{N}(x)$ is the standard normal distribution, then
\[
\left|F_n(x)-\mathcal{N}(x)\right| \leq 3 \rho / \sigma^3 \sqrt{n}
\]
\end{theorem}
