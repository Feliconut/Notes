...
The main reference in this note is \cite{durrettProbabilityTheory}.
...
\begin{proposition}
	If $f$ is a random process with independent increment, and is scale invariant in the sense that for any increments $\varepsilon$ and $\varepsilon'$, the scaled increments are equal in distribution, i.e. {\color{red}$\frac{f(x+\varepsilon) - f(x)}{\varepsilon} \stackrel{d}{=} \frac{f(x'+\varepsilon') - f(x')}{\varepsilon'}$ (need to be fixed)}, then for all $\delta>0$,
	we have the bound
	\[
...
\begin{proof}
	...
	\begin{align*}
		...
		&\ge\operatorname{Pr} \left[ \frac{\log \left| f(x') - f(x'+\varepsilon') \right|}{\log \varepsilon'} - \frac{\log C}{\log \varepsilon'} - \alpha < \delta \right]
	.\end{align*}
	Using the previous proposition, we can choose small enough $\varepsilon'$ to make this probability arbitrarily close to $1$.
...
\begin{theorem}
	Assume (some condition). Then for any specific $\varepsilon_j$,
\[
	\operatorname{inf} _{i \in [n]} a_i^{(j)} <
	 \operatorname{inf}_x
		\frac{\log \left| f(x) - f(x+\varepsilon_j) \right|}{\log \varepsilon_j} + O\left(\frac{1}{\log n}\right)
.\] 
\end{theorem}
\begin{proof}
	To be done. Need to work out what assumptions I need.
\end{proof}
...
\begin{equation}
	...
	\alpha + \frac{\log C}{\log \varepsilon_j} = \operatorname{inf} _i a_i^{(j)} + O\left(\varepsilon\right)+ O \left(\frac{1}{\log n}\right) 
.\end{equation} 

{\color{gray}More work needed to turn this into an error bound for $\alpha$, by considering the curve regression error.}
