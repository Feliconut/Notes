...
The bound improves as $K$ increases; and by taking large enough $K$ we can get a bound better than any polynomial in $n$.
...
\begin{lemma}
	...
	\[
		\mathbb{E}\left[ \left|   
			\sum_{j = \tau_{i-1}^{\mathcal{B}}-i}^{\tau_i^{\mathcal{B}}-i}   
			\alpha(i, j) 
		\right| ^K \right]
		= O\left(i^{-Kp - \frac{1}{2}K}\right)
	.\] 
