...
What remains to prove is that
		...
		First observe that $A^c$ can be decomposed to finitely many disjoint closed intervals $I_1, I_2, \ldots, I_n$ . By definition of $A^c$ we see that for each $I_k$ , $x(I_k)$ is either entirely positive or entirely negative.  
		Now  
		  \begin{align*}
...
\begin{proof}
	...
	\[
		\mathbb{E}\left[ \int _0^1 \mathbf{1}(X_t = 0) dt \right] = \int _0^1 \operatorname{Pr} \left[ X_t = 0 \right] dt = 0 
	.\] 
