...
\begin{equation}
	\label{eqn:2samp-bounds}
	 \int _0^d L(d-a) L(da) \le 
	 \operatorname{Pr} \left[ |D_{m, n} - \frac{\Delta}{\sqrt{n} }| < \frac{d}{\sqrt{n} }| H_0 \right] \le 
	 \int _0^\infty  \left[ L(a+d) - L(a-d) \right] L(da)
.\end{equation}


where $a$ is variable for integration. The left hand side is $0$ when $d < 0$.

For simplicity of notation, we define
\[
	A(d) := \int _0^d L(d-a) L(da)  \qquad
	B(d) := \int _0^\infty  \left( L(a+d) - L(a-d) \right) L(da)
.\] 

and write the inequality above as
\begin{equation}
	 A(d) \le 
	 \operatorname{Pr} \left[ |D_{m, n} - \frac{\Delta}{\sqrt{n} }| < \frac{d}{\sqrt{n} }| H_0 \right] \le 
	B(d) 
