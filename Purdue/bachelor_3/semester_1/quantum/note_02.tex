...
\textbf{Norm of a quantum state.} For orthogonal eigenstates $\left| + \right\rangle , \left| - \right\rangle $, a state $\left| \phi \right\rangle = a \left| + \right\rangle  + b \left| - \right\rangle $, $a, b \in \mathbb{C}$, has norm / length $|a|^2 + |b|^2$. For example,,
\[
	\left(\frac{2+\sqrt{2}}{\sqrt{2}}\right)|+\rangle+\left(\frac{1+i}{\sqrt{2}}\right)|-\rangle
.\] 
has norm
\[
	\left(\frac{2+\sqrt{2}}{\sqrt{2}}\right)^2+\left(\frac{(1 + i) ( 1 - i)}{2}\right)
.\]

\textbf{Matrix element:} The matrix element of operator $H$ in $\left| a_i \right\rangle $ basis, $a_i$ orthonormal, is $\left\langle a_j \right| H \left| a_i \right\rangle $.

\textbf{Change of basis:} Always we can insert another orthonormal (orthogonal and unit) basis $\sum_{b_j} \left| b_j \right\rangle \left\langle b_j \right| $. Thus
\[
	\left\langle a_j \right| H \left| a_i \right\rangle 
	= \sum_{b_k}
	\left\langle a_j \right| \left| b_k \right\rangle \left\langle b_k \right|  H \left| a_i \right\rangle 
.\] 
This changes from $a, a$ basis to $b, a$ basis. Also, feel free to do
\[
	\left\langle p \right| H \left| x \right\rangle = \int dx' \left\langle p \right|  \left| x' \right\rangle \left\langle x' \right| H \left| x \right\rangle 
.\] 
...
If there exists a complete set of mutually compatible observables $A_k$ that also commute with $H$, then $\mathcal{U}$ has a spectral decomposition: $\mathcal{U}_{t} = \exp\left( \frac{- i H t}{\hbar} \right) 
...
\textbf{Generation.} $A \to B$ means $A$ generates $B$ in the form $\exp\left( - \frac{i}{\hbar} A \Delta b \right) \left| b \right\rangle  = \left| b + \Delta b \right\rangle $. Momentum $\to $ position / translation; Angular momentum $J \cdot \hat{n}  \to $ angular position / rotation; Energy $ H \to $ time / time evolution.

\textbf{Addition of angular momentum.} Two angular momentums $(J_1^2, J_{1z})$ and $\left( J_2^2, J_{2 z} \right) $ can be added to form $J^2, J_z, J_1^2, J_2^2, J_{1z}, J_{2z}$. Those above are operators; their values denoted by $J, M, j_1, j_2, m_1, m_2$. \textbf{Two types of set of observables}: \[
	(J_1, J_2, J, J_z) \qquad (J_1, J_2, J_{1z}, J_{2z})
.\] 
\textbf{Example for reading Clebsch–Gordan coefficients table: Addition of angular momentum for spin-1/2 particles}. 
In this case we have $j_1 = j_2 = \frac{1}{2}$, corresponding to top-left corner. If $J = 1, M = 0$, then our state is  $\left| J=1, M = 0; j_1 j_2 \right\rangle  =  \frac{1}{\sqrt{2} }\left| m_1=\frac{1}{2}, m_2 = -\frac{1}{2}; j_1 j_2 \right\rangle  + \frac{1}{\sqrt{2} }\left| m_1 = -\frac{1}{2}, m_2 = \frac{1}{2}; j_1 j_2 \right\rangle .$

\textbf{Parity.} The \textbf{Spin Statistics Theorem} says fermions  (half integer spin particles) have odd symmetry, i.e. wave function takes a minus sign under exchange of particles. The parity is $-1$ / odd, if exchange gives a minus sign / particles anticommute. The parity is $1$ / even if particles commute.

Parity of state of a particle is product of parity of spin angular momentum $S$ state and parity of orbital angular momentum $L$ state.  $L$ is given by $(-1)^l$. Parity of $S$ state needs to be read from $\left| j_1, j_2, m_1, m_2 \right\rangle $ representation.



\textbf{Pauli exclusion principle} says two fermions cannot be in identical state. By "identical" we mean the same spatial state and same spin state. Example: for two electrons orbiting each other, they cannot both spin up, i.e. be in $\left| ++ \right\rangle  \equiv \left| m_1 = \frac{1}{2}, m_2 = \frac{1}{2}; j_1 = \frac{1}{2}, j_2 = \frac{1}{2} \right\rangle $ state, or both spin down. Example: for two electrons orbiting a nucleus, to have same spin quantum number $m_1 = m_2$, they must be in different orbits, i.e. have different orbital quantum number.

\textbf{2D rotation matrix} $\begin{bmatrix}
		\cos \theta & - \sin \theta \\
		\sin \theta &  \cos \theta \\
	\end{bmatrix}
.$ 

\textbf{Axial rotation for spin $\frac{1}{2}$ and Pauli quantum gates}: $D(\hat{x} , \pi) = - i X$, same for $Y$, $Z$. $X = 
	\begin{bmatrix}
		0 & 1 \\
		1 & 0 \\
	\end{bmatrix}
...
\textbf{Infinitesimal and finite operations}: When $\Delta \phi$ is small, we have $D(\hat{n} , \Delta \phi) := \exp\left( - \frac{i}{\hbar} J \cdot  \hat{n}  \Delta \phi \right) \approx 1 - \frac{i}{\hbar} J \cdot  \hat{n}  \Delta \phi$. To justify, use taylor expansion to first order.

\textbf{Rotation and Wigner d-matrix.} (textbook 3.203)
\[
\begin{aligned}
\mathscr{D}_{m^{\prime} m}^{(j)}(\alpha, \beta, \gamma) & =\left\langle j, m^{\prime}\left|\exp \left(\frac{-i J_z \alpha}{\hbar}\right) \exp \left(\frac{-i J_y \beta}{\hbar}\right) \exp \left(\frac{-i J_z \gamma}{\hbar}\right)\right| j, m\right\rangle \\
& =e^{-i\left(m^{\prime} \alpha+m \gamma\right)}\left\langle j, m^{\prime}\left|\exp \left(\frac{-i J_y \beta}{\hbar}\right)\right| j, m\right\rangle\\
& =e^{-i\left(m^{\prime} \alpha+m \gamma\right)}d_{m^{\prime} m}^{(j)}(\beta)
\end{aligned}
\]
\textbf{Representation of angular momentum operator.} $\left\langle x\left|L_z\right| \psi\right\rangle=-i \hbar \frac{\partial}{\partial \phi} \psi(x)$.
Proof: 
 Start from the polar coordinate transform: $\begin{cases}
...
-
  ...
  .
  \end{align*}
Thus,$1 - \frac{i}{\hbar} \left\langle \mathbf{x} \right| \varepsilon L_z \left| \psi \right\rangle
...
\textbf{Orbital angular momentum Ladders:} eqn. (3.222), 
...
\textbf{Time evolution for ensemble.} In Schodinger picture, $\rho(t)=\mathscr{U}\left(t, t_0\right) \rho\left(t_0\right) \mathscr{U}^{\dagger}\left(t, t_0\right)$. 

$$
\begin{aligned}
i \hbar \frac{\partial \rho}{\partial t} & =\sum_i w_i\left(H\left|\alpha^{(i)}, t_0 ; t\right\rangle\left\langle\alpha^{(i)}, t_0 ; t|-| \alpha^{(i)}, t_0 ; t\right\rangle\left\langle\alpha^{(i)}, t_0 ; t\right| H\right) \\
& =-[\rho, H]
\end{aligned}
$$
