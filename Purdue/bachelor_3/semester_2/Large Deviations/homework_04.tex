...
\begin{proof}
	...
	.\] 
	This gives the quadratic solution
	\[
...
\begin{proof}
	...
	.\] 
	We see that $D_\Lambda = (-\mu, \mu)$ which contains $0$ in its interior. Therefore the assumptions for the Gärtner-Ellis theorem are satisfied and we may apply the first part of the theorem. To apply the second part of the theorem, we may observe from the formula of $\Lambda(t)$ that it is (1) continuous, hence l.s.c., (2) differentiale, on $D_\Lambda$. It only remains to verify that it is (3) steep. To this end, we compute the derivative,
	\[
...
Therefore, both parts of the Gartner-Ellis theorem apply and we obtain an LDP for $Z_n$.
...
\begin{proof}
	...
	\begin{align*}
		...
		&\to \int _0^1 \log \frac{dx}{1 - 2 t x}\\
	...
	.\] 
	It blows up when $t \to \frac{1}{2}$, but goes to zero as $t \to  - \infty $. Hence $\Lambda$ is not steep. $\Lambda$ is not lower-semicontinuous because it is defined in a left-neighborhood of $\frac{1}{2}$ but not $t = \frac{1}{2}$, as $\log (1 - 2 t)$ is not defined at that point.

	Nevertheless, it still satisfies a large deviation principle. We can apply the first part of Gärtner-Ellis theorem. We have already verified that (1) $\Lambda(t)$ exist and (2) $0 \in \int (D_\Lambda) = (\to \infty , \frac{1}{2})$. So it has an LDP upper bound, and an LDP lower bound with open set $G$ replaced by $G \cap E$, where $E$ is the set of exposed points of $\Lambda^*$, where the exposing hyperplane is in $int (D_\Lambda)$.

	To verify that we indeed have an LDP lower bound, let's look at $\Lambda'(t)$. We see that this function is (1) continuously differentiable, (2) strictly convex. Then we try to show that 
\end{proof}
