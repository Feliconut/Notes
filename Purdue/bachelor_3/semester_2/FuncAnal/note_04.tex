...
\begin{proposition}
	\label{prop:SpectEssShift}
	The essential spectrum of $T_l, T_r: l_p \to l_p$, $T_b: l_p(\mathbb{Z}) \to l_p(\mathbb{Z})$, $T_a: L_p(\mathbb{R}) \to L_p(\mathbb{R}); a \neq 0$, for $1 \le p \le \infty $, is $\mathbb{T}$.
\end{proposition}
\begin{proof}
	Consider the Calkin algebra, ref. Proposition~\ref{prop:EssSpecCalkin}. We have $T^* T$ either is identity or is projection of codimension $1$. Thus $T + \mathcal{K}$ is unitary in the Calkin algebra. Then we have $\sigma(T + \mathcal{K}) \subset \mathcal{T}$. In view of Lemma~\ref{lem:ShiftTopEquiv}, and that the spectrum is nonempty, we have $\sigma_e(T) = \sigma(T + \mathcal{K}) = \mathcal{T}$.
\end{proof}
...
\subsubsection{Norm of integration operator}
